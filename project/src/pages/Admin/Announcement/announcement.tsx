...
const AnnouncementContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://160.19.166.155:8000/admin/listAnnouncement");
  ...
  const handleDelete = async (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`http://160.19.166.155:8000/admin/listAnnouncement/${id}`, {
            method: "DELETE",
            credentials: "include",
          });
          if (!response.ok) {

...
const EditPtn = () => {
  ...
  useEffect(() => {
    const fetchPaket = async () => {
      try {
        const response = await fetch(`https://kawalptn.store/admin/viewUniversity/${id_ptn}`);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`https://kawalptn.store/admin/editUniversity/${id_ptn}`, {
            method: "PUT",
            headers: {

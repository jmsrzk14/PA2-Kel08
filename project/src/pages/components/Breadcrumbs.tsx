...
const Breadcrumbs: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPackageName = async (id: string) => {
      try {
        const response = await fetch(`http://160.19.166.155:8000/admin/viewPacket/${id}`);
        if (response.ok) {
    ...
    const fetchPtnName = async (id: string) => {
      try {
        const response = await fetch(`http://160.19.166.155:8000/admin/viewUniversity/${id}`);
        if (response.ok) {
          const data = await response.json();
    ...
    const fetchMajorName = async (id: string) => {
      try {
        const response = await fetch(`http://160.19.166.155:8000/admin/viewMajor/${id}`);
        if (response.ok) {
          const data = await response.json();
    ...
    const fetchSchoolName = async (id: string) => {
      try {
        const response = await fetch(`http://160.19.166.155:8000/admin/viewSekolah/${id}`);
        if (response.ok) {
          const data = await response.json();
    ...
    const fetchAnnouncementName = async (id: string) => {
      try {
        const response = await fetch(`http://160.19.166.155:8000/admin/viewAnnouncement/${id}`);
        if (response.ok) {
          const data = await response.json();
    ...
    if (pathnames.length > 2 && pathnames[1] === "school" && pathnames[2] === "viewSchool") {
    ...
    <div className="px-4 py-2 bg-gray-50">
      <div className="flex items-center text-sm">
        ...
        {pathnames.slice(1).map((name, index) => {
          ...
          if (name.match(/\d+/) && schoolName && pathnames[index] === "editSchool" ) {
